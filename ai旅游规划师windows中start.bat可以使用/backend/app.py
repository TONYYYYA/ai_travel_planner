...
from speech_service import speech_to_text, audio_stream, speech_to_text1, start_recording1, stop_recording1
...
# 开始录音（使用speech_to_text1）
@app.route('/api/speech_to_text1/start', methods=['POST'])
def start_speech_recognition():
    try:
        # 调用speech_service中的start_recording1函数
        start_recording1()
        return jsonify({"success": True, "message": "录音已开始"})
    except Exception as e:
        logger.error(f"启动录音失败: {str(e)}")
        return jsonify({"error": f"启动录音失败: {str(e)}"}), 500

# 停止录音并获取识别结果（使用speech_to_text1）
@app.route('/api/speech_to_text1/stop', methods=['POST'])
def stop_speech_recognition():
    try:
        # 调用speech_service中的stop_recording1函数获取结果
        results = stop_recording1()
        return jsonify({"text": results, "success": True})
    except Exception as e:
        logger.error(f"停止录音或获取结果失败: {str(e)}")
        # 即使出错，也返回一个友好的错误消息格式，与前端预期的格式保持一致
        return jsonify({
            "text": [{"result": {"ws": [{"cw": [{"w": f"语音识别错误: {str(e)[:100]}..."}]}]}}],
            "success": False,
            "error": str(e)
        })
